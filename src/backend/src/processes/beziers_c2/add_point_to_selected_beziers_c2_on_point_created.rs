...
pub struct AddPointToSelectedBeziersC2OnPointCreated {
    pub backend: Rc<RefCell<Backend>>,
}

impl Consumer<PointCreated> for AddPointToSelectedBeziersC2OnPointCreated {
    fn consume(&self, event: &PointCreated) {
        ...
        storage
            .selected_objects
            .iter()
            .filter(|object| object.bezier_c2_id.is_some())
            .for_each(|object| {
                ...
                unsafe {
                    let backend = &*backend;
                    let event =
                        Rc::new(PointAddedToBezierC2::new(event.id, id, event.name.clone()));
...
impl AnyConsumer for AddPointToSelectedBeziersC2OnPointCreated {
