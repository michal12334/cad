...
impl Command<AddPointToBezierC0> for AddPointToBezierC0 {
    fn execute(command: &AddPointToBezierC0, app_state: Rc<RefCell<Backend>>) {
        ...
        bezier.add_point(BezierC0Point { id: command.point_id });
        let point = backend.storage.points.get(&command.point_id).unwrap();
        let point_name = point.name.clone();
        drop(backend);
        let backend = app_state.borrow();
        let point_added_to_bezier = Rc::new(PointAddedToBezierC0::new(command.point_id, command.id, point_name));
        backend.services.event_publisher.publish(point_added_to_bezier);
    }
}
