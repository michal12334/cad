...
pub trait Command<TCommand> {
    fn execute(command: &TCommand, app_state: Rc<RefCell<AppState>>);
}

pub trait Query<TQuery, TResult> {
    fn get(query: &TQuery, app_state: Rc<RefCell<AppState>>) -> TResult;
}

pub trait Operation<TOperation, TResult> {
    fn handle(query: &TOperation, app_state: Rc<RefCell<AppState>>) -> TResult;
}
