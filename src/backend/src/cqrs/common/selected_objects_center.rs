...
impl Query<SelectedObjectsCenter, Option<LittleTransformerDTO>> for SelectedObjectsCenter {
    fn get(query: &SelectedObjectsCenter, app_state: &AppState) -> Option<LittleTransformerDTO> {
        if app_state.storage.selected_objects.len() >= 2 {
            app_state.storage.selected_objects.iter()
                .flat_map(|object| {
                ...
                .reduce(|x, y| (
                    ...
                    LittleTransformerDTO {
                        position: (x.1.position.0 + y.1.position.0, x.1.position.1 + y.1.position.1, x.1.position.2 + y.1.position.2),
                    }
                ))
                .map(|x| LittleTransformerDTO {
                    position: (x.1.position.0 / x.0 as f64, x.1.position.1 / x.0 as f64, x.1.position.2 / x.0 as f64),
                })
        } else {
            None
        }
