...
impl Command<MoveBezierC2SelectedBernsteinPoint> for MoveBezierC2SelectedBernsteinPoint {
    fn execute(command: &MoveBezierC2SelectedBernsteinPoint, app_state: Rc<RefCell<Backend>>) {
        ...
        let bezier = backend.storage.beziers_c2.get_mut(&command.bezier_id).unwrap();
        let point = bezier.get_point_movement_to_move_selected_bernstein_point(LittleTransformer { position: command.transformer.position, });
