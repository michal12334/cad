...
pub struct RebuildStorageOnSelectedPointsMerged {
    pub drawing_storage: Rc<RefCell<DrawingStorage>>,
    pub cqrs: CQRS,
    pub display: Rc<Display<WindowSurface>>,
}

impl Consumer<SelectedPointsMerged> for RebuildStorageOnSelectedPointsMerged {
    fn consume(&self, _: &SelectedPointsMerged) {
        ...
        for bezier_c0 in self.cqrs.get(&AllBeziersC0 {}) {
            ...
            drawing_storage.beziers_c0.insert(
                bezier_c0.id,
                BezierC0::new(bezier_c0.id, &points, &self.display),
            );
        }
        ...
        for bezier_int in self.cqrs.get(&AllBeziersInt {}) {
            let points = self
                .cqrs
                .get(&BezierIntBernsteinPoints { id: bezier_int.id });
            drawing_storage.beziers_int.insert(
                bezier_int.id,
                BezierInt::new(bezier_int.id, &points, &self.display),
            );
        }
...
impl AnyConsumer for RebuildStorageOnSelectedPointsMerged {
