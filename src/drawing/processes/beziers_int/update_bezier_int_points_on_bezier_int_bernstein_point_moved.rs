...
impl Consumer<BezierIntBernsteinPointMoved> for UpdateBezierIntPointsOnBezierIntBernsteinPointMoved {
    fn consume(&self, event: &BezierIntBernsteinPointMoved) {
        let mut drawing_storage = self.drawing_storage.borrow_mut();
        let bezier = drawing_storage.beziers_int.get_mut(&event.bezier_id).unwrap();
        let points = self.cqrs.get(&BezierIntBernsteinPoints { id: event.bezier_id });
