...
pub struct UpdateSurfaceC2Texture {
    pub id: u64,
}

pub struct UpdateSurfaceC2TextureConsumer {
    pub drawing_storage: Rc<RefCell<DrawingStorage>>,
    pub cqrs: CQRS,
    pub display: Rc<Display<WindowSurface>>,
}

impl Consumer<UpdateSurfaceC2Texture> for UpdateSurfaceC2TextureConsumer {
    fn consume(&self, message: &UpdateSurfaceC2Texture) {
        let mut drawing_storage = self.drawing_storage.borrow_mut();
        let surface = drawing_storage.surfaces_c2.get_mut(&message.id).unwrap();
        let texture_data = self.cqrs.get(&IntersectionObjectTexture {
            id: IntersectionObjectIdDTO::SurfaceC2(message.id),
        });
...
impl AnyConsumer for UpdateSurfaceC2TextureConsumer {
